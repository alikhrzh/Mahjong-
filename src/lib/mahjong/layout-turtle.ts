/** Classic turtle layout (x,y,z), one tile per coordinate triple. */
export const TURTLE_LAYOUT_RAW = `0,7,0;
...
export function parseLayout(raw: string): Array<{ x: number; y: number; z: number }> {
  return raw
    .split(/[;\n]+/)
    .map((s) => s.trim())
    .filter(Boolean)
    .map((triple) => {
      const [x, y, z] = triple.split(",").map((n) => Number(n.trim()));
      if (![x, y, z].every(Number.isFinite)) throw new Error(`Bad layout token: ${triple}`);
      return { x, y, z };
    });
}
